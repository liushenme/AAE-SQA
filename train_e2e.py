...
from model import e2e_make_model_and_optimizer, e2e_SimpleSystem
import sys
...
def main(conf):
    ...
    train_set = E2EDataset(os.path.join(conf["data"]["json_dir"], "train/"))
    print(len(train_set))
    val_set = E2EDataset(os.path.join(conf["data"]["json_dir"], "val/"))
    ...
    #loss_func = get_loss_func('clip_frame_mse_new')
    system = e2e_SimpleSystem(
        model=model,
        loss_func=loss_func,
        optimizer=optimizer,
...
if __name__ == "__main__":
    ...
    with open("local/conf_e2e_qq.yml") as f:
    #with open("local/conf_e2e_wb_new_dnsmos.yml") as f:
    #with open("local/conf_e2e_qq_dnsmos.yml") as f:
